...
[cat-sdk](../README.md) / mintNft

# Function: mintNft()

> **mintNft**(`signer`, `provider`, `minterUtxo`, `proof`, `proofNodePos`, `nextMerkleRoot`, `nft`, `collectionId`, `metadata`, `nftReceiver`, `changeAddress`, `feeRate`): `Promise`\<\{ `createNftPsbt`: `ExtPsbt`; `createNftTxId`: `string`; `mintPsbt`: `ExtPsbt`; `mintTxId`: `string`; \}\>
