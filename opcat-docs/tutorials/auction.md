...
const deployTx = await auction.deploy(signer, provider, auction);
```
...
const refundOutput: ByteString = TxUtils.buildP2PKHOutput(
  highestBid,
  highestBidder
);
```
...
public bid(bidder: Addr, bid: bigint) {
    ...
    // Refund previous highest bidder.
    const refundOutput: ByteString = TxUtils.buildP2PKHOutput(highestBid, highestBidder)
...
export class Auction extends SmartContract {
  ...
  public bid(bidder: PubKey, bid: bigint) {
    ...
    // Add change output.
    outputs = auctionOutput + this.buildChangeOutput();
